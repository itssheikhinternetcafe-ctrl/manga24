...
export const WriterDashboardPage: React.FC = () => {
  ...
  const handlePagesUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const urls: string[] = [];
      for (const file of files) urls.push(await uploadMediaFile(file, 'writer_chapters'));
